...
const RecipeDetailedCard = ({ onClose }: { onClose: () => void }) => {
  const article = RecipeGlobalState((state) => state.article as CardTypes);
    ...
    <div className="fixed top-0 left-0 w-full h-full bg-black bg-opacity-50 flex justify-center items-center">
      <div className="bg-white p-8 rounded-lg max-w-lg overflow-y-auto">
        ...
        <div className="flex flex-col items-center">
          <img src={article.imageUrl} alt="Dish" className="w-64 h-64 object-cover mb-4" />
        ...
        <div className="flex justify-center">
          ...
          <button className="ml-2 bg-yellow-500 text-white py-2 px-6 rounded-md shadow-md hover:bg-yellow-600 transition duration-300 ease-in-out text-sm" onClick={onClose}>Back</button>
        </div>
      </div>
    </div>
  );
